...
class EMB(object):
    ...
    def load_embed(self):
        ...
        if self.embed_model in ['DistMult', 'TransE', 'ComplEx', 'RESCAL']: 
            print(self.dataset + '/entity2vec.' + self.embed_model)
            ...
            self.symbol2id = symbol_id  # 69126+1
            self.symbol2vec = embeddings # 69126+1
            return self.symbol2id, self.symbol2vec

    def build_connection(self, max_=100): # Construct the connection information of the whole graph.
        ...
        return self.e1_rele2

    def get_meta(self, left, right):# left [67857, 19816, 57126, 32156, 42330]
...
class Trainer(object):
    def __init__(self, arg):
        ...
        self.embport= EMB(args)
      
        self.symbol2id, self.symbol2vec = self.embport.load_embed()
        self.num_symbols = len(self.symbol2id.keys()) - 1  # one for 'PAD' 
        self.pad_id = self.num_symbols
        self.e1_rele2 = self.embport.build_connection()
        ...
        self.Matcher = Matcher(self.embed_dim, self.num_symbols,
                               ...
                               device=self.device,                  
                               symbol2id = self.symbol2id, symbol2vec = self.symbol2vec,e1_rele2 = self.e1_rele2,
                               )
    ...
    def train(self):
        ...
        for data in train_generate(self.dataset, self.batch_size, self.train_few, self.symbol2id, self.ent2id,
            ...
            self.batch_nums += 1
            support_meta = self.embport.get_meta(support_left, support_right)
            query_meta = self.embport.get_meta(query_left, query_right)
            false_meta = self.embport.get_meta(false_left, false_right)
    ...
    def test_(self, path=None):
        self.load(path)
        logging.info('Pre-trained model loaded for test set')
        self.eval(mode='test', meta=self.meta) #
    def eval_(self, path=None):
        self.load(path)
        logging.info('Pre-trained model loaded for dev set')
